...
void evaluate(cars_t *unsorted_data, keys_t *unsorted_keys)
{
    ...
    printf("Тестирование скорости сортировок для %d повторений и %zu элементов:\n", EVALUATE_COUNT, data->amount);

    printf("%s - %ld; Объём памяти: %zu\n", "Быстрая сортировка (основная таблица) ", dur_fs_main / EVALUATE_COUNT, fs_main_size);
    printf("%s - %ld; Объём памяти: %zu\n", "Быстрая сортировка (таблица ключей)   ", dur_fs_key / EVALUATE_COUNT, fs_key_size);

    printf("%s - %ld; Объём памяти: %zu\n", "Сортировка выбором (основная таблица) ", dur_ss_main / EVALUATE_COUNT, ss_main_size);
    printf("%s - %ld; Объём памяти: %zu\n", "Cортировка выбором (таблица ключей)   ", dur_ss_key / EVALUATE_COUNT, ss_key_size);
...
int search(const cars_t *data)
{
    ...
    int rc;
    printf("Поиcк поддержанных автомобилей указанного бренда с одним предыдущим \n"
