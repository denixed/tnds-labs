 ...
 */
static void *get_min_pos(void *arr_begin, void *arr_end, size_t size,
                         int (*cmp)(const void *, const void *)) {
  void *min_ptr = arr_begin;
  for (char *i_ptr = (char *)arr_begin + size; (void *)i_ptr < arr_end;
       i_ptr += size)
    if (cmp(i_ptr, min_ptr) < 0) min_ptr = i_ptr;
 ...
 * @param cmp
 */
void selection_sort(void *array, size_t nmemb, size_t size,
                    int (*cmp)(const void *, const void *)) {
