...
2.  **Целое число, представляющее собой номер команды:** целое число в диапазоне от **0 **до **15**.
...
typedef struct
{
    char marque[LEN_MARQUE]; // Бред автомобиля
...
Для разных состояний автомобиля использыются типы `pre_owned_attr_t` и `new_attr_t`
...
typedef struct
{
    ...
    uint16_t num_of_repairs; // Колличесто ремонтов 
    uint16_t num_of_owners; // Колличество владельцев
...
> А какие тесты в этой лабе с таким ГУИ?
...
|Количество элементов| Время (`сlock_t`) (осн)|  Память (байт) (осн)| Время (`сlock_t`) (доп)|  Память (байт) (доп)|
|---|---|---|---|---|
