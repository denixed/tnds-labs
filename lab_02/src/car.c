...
{
    return strcasecmp(((short_car_info_t *) a)->marque, ((short_car_info_t *) b)->marque);
}
...
void print_cond_attr(const car_t *a)
{
    if (a->condition == NEW)
        ...
        printf("%15s|\n"
               ...
               "Old",
               "Product year:", a->cond_attr.for_pre_owned.product_year, 
...
{
    printf("|%15s|%15s|%10" PRIu64 " rub.|%15s|",
           a->marque,
           a->country,
           a->price,
           a->color);
...
int input_price(car_t *car)
{
    ...
    if (!fgets(temp_price, sizeof(temp_price), stdin))
        return ERR_READ_CAR_PRICE;
    if (check_int(temp_price) != OK)
        return ERR_READ_CAR_PRICE;
    if (!strlen(temp_price))
        return ERR_EMPTY_INPUT;
...
int input_color(car_t *car)
{
    ...
    if (!fgets(car->color, sizeof(car->color), stdin))
...
int input_condition(car_t *car)
{
    ...
    printf("Выберите состояние автомобиля:\n"
           "0. Новый\n"
           "1. Подержанный\n"
           "Введите <0> или <1>"
           " и нажмите <Enter>): ");
    if (!fgets(_tmp_, sizeof(_tmp_), stdin))
        return ERR_READ_CAR_CONDITION;
    if (check_int(_tmp_) != OK)
        return ERR_READ_CAR_CONDITION;
    if (!strlen(_tmp_))
        return ERR_EMPTY_INPUT;
...
int input_cond_attr(car_t *car)
{
    ...
    */
    ...
    if (car->condition == NEW)
    {
        ...
        if (!fgets(_int_8_, sizeof(_int_8_), stdin))
            return ERR_READ_CAR_COND_ATTR;
        if (check_int(_int_8_) != OK)
            return ERR_READ_CAR_COND_ATTR;
        if (!strlen(_int_8_))
            return ERR_EMPTY_INPUT;
    ...
    else
    {
        ...
        if (!fgets(_int_32_, sizeof(_int_32_), stdin))
            return ERR_READ_CAR_COND_ATTR;
        if (check_int(_int_32_) != OK)
            return ERR_READ_CAR_COND_ATTR;
        if (!strlen(_int_32_))
            return ERR_EMPTY_INPUT;
        car->cond_attr.for_pre_owned.milage = (uint32_t)atol(_int_32_);

        printf("Введите количество ремонтов (0-9999) автомобиля и нажмите <Enter>: ");
        if (!fgets(_int_16_, sizeof(_int_16_), stdin))
            return ERR_READ_CAR_COND_ATTR;
        if (check_int(_int_16_) != OK)
            return ERR_READ_CAR_COND_ATTR;
        ...
        printf("Введите количество предыдущих владельцев автомобиля (0-9999) и нажмите <Enter>: ");
        if (!fgets(_int_16_, sizeof(_int_16_), stdin))
            return ERR_READ_CAR_COND_ATTR;
        if (check_int(_int_16_) != OK)
            return ERR_READ_CAR_COND_ATTR;
